        ...
        const loadThumbnails = async () => {
            ...
            for (const song of album.songs) {
                if (song.thumbnailUrl) {
                    try {
                        const { url, cleanup } = await apiClient.getStream(
                            `/api/songs/thumbnails/${song.thumbnailUrl}`,
                        );
                        urls[song.id] = url;
                        cleanupFunctions.push(cleanup);
                    } catch (error) {
                        console.error(
                            `Failed to load thumbnail for song ${song.id}:`,
                            error,
                        );
        ...
        <div className="album-container">
            <div className="album-header">
                <img
                    src={
                        album.songs[0]
                            ? thumbnailUrls[album.songs[0].id]
                            : getAssetPath('images/placeholder.jpg')
                    }
                    alt={album.name}
