...
const convertSongRow = (row: SongRow): {
  duration: number;
  filepath: string;
  artist: string;
  genre: string;
...
export class DbClient {
  ...
  async getUniqueAlbums(): Promise<string[]> {
    return new Promise((resolve, reject) => {
      this.db.all<AlbumRow>('SELECT DISTINCT albumId FROM songs', [], (err, rows) => {
        if (err) reject(err);
  ...
  async getUniqueArtists(): Promise<string[]> {
    return new Promise((resolve, reject) => {
      this.db.all<ArtistRow>('SELECT DISTINCT artist FROM songs', [], (err, rows) => {
        if (err) reject(err);
  ...
  async getSongById(id: number): Promise<Song | null> {
    return new Promise((resolve, reject) => {
      this.db.get<SongRow>('SELECT * FROM songs WHERE id = ?', [id], (err, row) => {
        if (err) reject(err);
